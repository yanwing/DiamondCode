...
#ifndef IRRADIATED_finestrip_ANALYSIS_C
#define IRRADIATED_finestrip_ANALYSIS_C

void irradiated_finestrip_analysis(int structure, bool on)
  
{
  ...
  if(on)
    {
      strcture_firstfile[3]  = 390;
      strcture_lastfile[3]   = 523;
...
if(structure == 3 && !on)
  ...
  */
  ...
  // Create File
  TFile* File = new TFile(Form("../data/Irradiated_finestrip_Structure_%d_%s.root",structure, onoffstring.c_str()),"RECREATE");
  TFile* RawData = 0;
  
  cout << "After creating the output file" << endl;
  ...
  cout << "after Noise Histo" << endl;
  // Create Hitmaps
  
  //Laura (with some help from Root forum):
  TDirectory *dir = File->GetDirectory(Form("Irradiated_finestrip_Structure_%d_%s",structure, onoffstring.c_str()));
  if (dir) {
    dir->cd();
  } else {
    ...
    File->mkdir(Form("Irradiated_finestrip_Structure_%d_%s",structure, onoffstring.c_str()));
    File->cd(Form("Irradiated_finestrip_Structure_%d_%s",structure, onoffstring.c_str()));
  }

  cout << "after creating directory and file" << endl;
  ...
  if(structure == 3 || structure == 10 || structure == 12 || structure == 8)
  {
      ...
      for(int k = 0; k < 134; k++)
	{
	  ...
	  hit_strip = hit_stcout << "hit_strip" <<rip_off_3[k];
	  ...
	  */
	  Hitmap[k]= new TH2F(Form("Hitmap_Structure_%d_Beamposition_%d",structure,k), Form("Hitmap (%s), beam position  %d; Channel ; Threshold in [mV]",struc_ID[structure].c_str(),k),128,-0.5,127.5, 16,48.8,99.2); //Laura
  ...
  for(int x = 1; x <= 12; x++)
      ...
      for(int l = strcture_firstfile[structure]; l <= strcture_lastfile[structure]; l++) // loop over files
	{
		 ...
		 if(n >= strip_ID[x] && n < last_channel) // the 128 channels of a structure
		   {
		     ...
		     if(bin_content != 0)
		       {
			 //bin_error = 1/TMath::Sqrt(bin_content);//wrong error calculation
			 bin_error = TMath::Sqrt(bin_content);
		 	 
			 // fill hitmaps
			 if(x == structure)
			   {
			     Hitmap[m]->Fill(channel,threshold_step,bin_content);
			     Hitmap[m]->SetBinError(channel,p,bin_error);//should be automatically set when sumw2 is called

			     //  cout << "filled Hitmap for structure " << structure << endl;
			 ...
			 // fill noise-maps
			 else
			   {
			     ...
			     //Average_Noise_Histo[q]->SetBinError(channel,p,bin_error);//don't use SetBinError for profiles
			   }
		       }
		     else continue;
		   }
	 ...
	 cout <<"closed raw data file of structure " << x << " file " << l <<endl;
	} // close file loop
     
      cout << "end of loop over all structures" << endl;
    }
...
#endif /* IRRADIATED_finestrip_ANALYSIS_C */
