  ...
  
{
  ...
  // Create File
  TFile* File = new TFile("../data/Noise.root","RECREATE");
  TFile* RawData = 0;
  ...
  for(int i = 0; i < 8; i++)
    {
      ...
      RawData = TFile::Open(Form("../data/Irradiated_Structure_%s.root",Data_ID[j].c_str()));
      ...
      for(int k = 0; k < 12; k++)
 	{
	  ...
	  Noise_Histo[k]->Add(Raw_Histo);  
	  if (l == 3)
	    {
	      cout << "scan " << j << " (of 8), structure " << l << " (of 12), Noise_Histo bin (1,1) content: " << Noise_Histo[k]->GetBinContent(1,1)<< ", Raw_Histo " << Raw_Histo -> GetBinContent(1,1) << endl; 
